...
RSpec.describe AppChain::Contract do
  let(:abi_str) do
  ...
  # let(:appchain) { AppChain::Client.new("http://121.196.200.225:1337") }
  let(:appchain) { AppChain::Client.new("http://example.com") }
  let(:contract) { appchain.contract_at(abi_str, "0x3259704dff6ba09184e1fcb2c47969b4fafffb7f") }
  ...
  context "call_func" do
    it "symbol" do
      allow(appchain.rpc).to receive(:call_rpc).with(:call, params: [{ data: symbol_data, to: contract_addr }, "latest"]).and_return({
                                                                                                                                   "jsonrpc" => "2.0",
                                                                                                                                   "id" => 83,
    ...
    it "transfer" do
      allow(appchain.rpc).to receive(:call_rpc).with(:call, params: [{ data: transfer_data, to: contract_addr }, "latest"]).and_return({
                                                                                                                                     "jsonrpc" => "2.0",
                                                                                                                                     "id" => 83,
                                                                                                                                     "result" => "0x"
                                                                                                                                   })
    ...
    context "parse abi" do
      ...
      let(:contract) { appchain.contract_at(abi, "0x3259704dff6ba09184e1fcb2c47969b4fafffb7f") }
  ...
  context "send_func" do
    before do
      allow(appchain.rpc).to receive(:call_rpc).with("sendRawTransaction", params: [""]).and_return({
               "jsonrpc" => "2.0",
               "id" => 83,
               "result" => {
    ...
    it "transfer with transaction" do
      transaction = AppChain::Transaction.from_hash(tx)
