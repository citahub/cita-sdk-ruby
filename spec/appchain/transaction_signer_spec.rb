RSpec.describe AppChain::TransactionSigner do
  ...
  context "encode" do
    context "test transfer" do
      ...
      it "encode correct" do
        tx = AppChain::Transaction.new(tx_hash)
        result = AppChain::TransactionSigner.encode(tx, private_key)
    ...
    let(:expect_data) do
      {
        unverified_transaction: {
          transaction: {
            ...
            quota: 1_000_000,
            valid_until_block: 999999,
            data: AppChain::Utils.to_bytes(data),
            value: AppChain::TransactionSigner.send(:hex_to_bytes, "0x0"),
            chain_id: 1,
            version: 0
          },
        ...
        let(:expected_data) do
          {
            unverified_transaction: {
              transaction: {
                ...
                valid_until_block: 91789,
                data: data,
                value: AppChain::TransactionSigner.send(:hex_to_bytes, "0x3e8"),
                chain_id: 1,
                version: 0,
              },
        ...
        let(:expected_data) do
          {
            unverified_transaction: {
              transaction: {
                ...
                data: data,
                value: AppChain::TransactionSigner.send(:hex_to_bytes, "0x3e8"),
                chain_id: AppChain::TransactionSigner.send(:hex_to_bytes, "0x1"),
                version: 1
              },
